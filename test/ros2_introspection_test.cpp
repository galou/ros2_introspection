...
TEST(Ros2Introspection, Imu) {
  ...
  parser.deserializeIntoFlatMessage("imu", &serialized_msg.get_rcl_serialized_message(),
                                    &flat_message, 100);
...
TEST(Ros2Introspection, Polygon) {
  ...
  parser.deserializeIntoFlatMessage("poly", &serialized_msg.get_rcl_serialized_message(), &flat_message, max_array_size);
...
TEST(Ros2Introspection, Battery) {
  ...
  const std::string topic_type = "sensor_msgs/BatteryState";
  ...
  parser.deserializeIntoFlatMessage("battery", &serialized_msg.get_rcl_serialized_message(),
                                    &flat_message, 100);

  ConvertFlatMessageToRenamedValues(flat_message, renamed);

  for(const auto& [name, value]: renamed)
  {
...
TEST(Ros2Introspection, Image) {
  ...
  const bool has_blobs = parser.deserializeIntoFlatMessage("image", &serialized_msg.get_rcl_serialized_message(), &flat_message, 100);
  ASSERT_TRUE(has_blobs);
